...
Before coding or style answers: find with narrow scope (tags AND) + query when shelves is on → { rules with resolved_sources, documents, links }. When local-language search terms differ from query, pass query_local alongside query (both run BM25 on vault and shelves; merged by max score). Same-turn add/supersede/reinforce may persist query_local on the rule. add/supersede claim should be English imperative; user verbatim → text; local search terms → query_local. Link kinds in find.links (session-only except sources field): sources (vault rule→doc), cited_by ([[r-…]] in doc), co_search (same-query BM25; session-only).
