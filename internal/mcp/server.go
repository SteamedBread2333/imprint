...
Before coding or style answers: find with narrow scope (tags AND) + query when shelves is on → { rules with resolved_sources, documents, links }. When local-language terms differ from query, pass query_local too (dual BM25; max score merge). Answer from rule claims first; documents are supplemental. add: English claim; text = user verbatim; query_local optional (vault merges CJK evidence tokens). Omit confidence on add (0.6); never 0.9 on add. sources only when doc excerpt substantively supports the rule—not topical overlap. Weak docs/co_search links are filtered from find results.
