...
6. User asks what is recorded / to see the picture → `show`; if more than ~20 items, `viz` (`--format mermaid` or `--format notes`) or suggest `imprint desk open` for the graph UI.
