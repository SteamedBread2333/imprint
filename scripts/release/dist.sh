...
for spec in "${PLATFORMS[@]}"; do
  ...
  docs=()
  for f in LICENSE README.md README.zh.md docs/PROMPT-v2.1-legacy.md docs/mcp.md docs/mcp.zh.md docs/correction.md docs/correction.zh.md docs/editors.md docs/editors.zh.md; do
    if [[ -f "$f" ]]; then
      base="$(basename "$f")"
      cp "$f" "${stage}/${base}"
      docs+=("$base")
