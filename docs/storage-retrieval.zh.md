...
**轻量本地查询**：CLI `find` 仅访问结构化记忆库（支持 `--query-local` 双路 merge），不返回文档片段与当次关联。
...
1. 若有段落 ID → 在索引中精确查找；
2. 否则路径 + 章节标题：markdown inline 转为纯文本后精确匹配，失败则双向子串；
3. 文件在索引中但标题仍对不上 → **`heading_unresolved: true`**，保留声明 heading，**不返回其他章节摘录**（禁止静默回退文首）；
4. 路径不在索引 → 标记「超出索引范围」`out_of_index`；
5. 段落 ID 已失效 → 回退路径/标题（同 2–3）；仅有 path、无 heading → 取该文件第一个 chunk。

标题比较：code span、链接只保留可见文字、emphasis、反斜杠转义，再去掉 `5.2` 这类节号。不跟随链接目标，也不对标记字符做全局替换。
